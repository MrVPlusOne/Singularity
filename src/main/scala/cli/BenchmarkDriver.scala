...
object BenchmarkDriver {
  ...
  val benchmarks = {
    ...
    Map[String, CliOption => FuzzingTaskProvider](
      ...
      "hash/java" -> (opt => hashCollisionExample(HashFunc.java)),
      "hash/ruby" -> (opt => hashCollisionExample(HashFunc.ruby)),
      "hash/asp.net" -> (opt => hashCollisionExample(HashFunc.aspDotNet)),
      "hash/python" -> (opt => hashCollisionExample(HashFunc.python)),
      "hash/v8" -> (opt => hashCollisionExample(HashFunc.v8)),
      "hash/murmur2seed0" -> (opt => hashCollisionExample(HashFunc.murmur2(0))),
